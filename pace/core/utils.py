    ...
    if bundle[0] == 'O':
        ...
        if c[0]=='i':
            ...
            if t2 == 'B':
                ...
                buf.write('\x0A')
                (p2, p3, p4) = m2
                buf.write(struct.pack('BBBB', p1, p2, p3, p4))
                buf.seek(0)
                return buf.read()
            else:
                raise deepEncodeException()
            (p2, p3) = m2
            #print p1, p2, p3, type(p1), type(p2), type(p3)
...
def deserializeEnc(r):
    '''print "\n\n**********************************"
    print deserialize(r[EC_SERIALIZED_1+EC_LABEL_LENGTH: EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2])
    print deserialize(r[EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2:EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2*2])
    print deserialize(r[EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2*2:EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2*3])
    print deserialize(r[EC_SERIALIZED_1+EC_LABEL_LENGTH+EC_SERIALIZED_2*3:ENC_SERIALIZED_LENGTH])'''
...
def deepDecode(m, msgTypeCounter):
    ...
    if msgtype == 1:
        ...
        p1, p2, p3, p4 = struct.unpack('BBBB', buf.read(4))
        return mc, (f, t, ('A', (p1, ('E', (p2, p3, p4)))),)
    elif msgtype == 11:
        p1, p2, p3 = struct.unpack('BBB', buf.read(3))
        return mc, (f, t, ('A', (p1, ('G', (p2, p3)))),)
    elif msgtype == 12:
        p1, p2, p3 = struct.unpack('BBB', buf.read(3))
        return mc, (f, t, ('A', (p1, ('H', (p2, p3)))),)
    else:
