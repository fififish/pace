...
def multiSigBr(pid, N, t, msg, broadcast, receive, outputs, send):
    ...
    def merkleTree(strList, someHash = coolSHA256Hash):
        ...
        #print "okoko", treeLength
        #print len(strList)
        for i in range(len(strList)):
            mt[i + treeLength] = someHash(strList[i])  # TODO: need to change strList[i] from a string to an integer here.
        if len(strList)<treeLength:
            for i in range(len(strList),treeLength):
                mt[i+treeLength] = someHash('0')
        for i in range(treeLength - 1, 0, -1):  # 1, 2, 3, ..., treeLength - 1
            ...
            #print "!!!!!!!!!!!!!", type(mt[i*2]), type(mt[i*2+1]), i*2, i*2+1
            
            mt[i] = someHash(mt[i*2] + str(mt[i*2+1]))  # concat is not commutative
        return mt
...
@greenletFunction
def includeTransaction(pid, N, t, version, setToInclude, broadcast, receive, send):
    CBChannel = Queue()
    ACSChannel = Queue()
        ...
        while True:
            ...
            if tag == 'B':
                greenletPacker(Greenlet(CBChannel.put, (sender, m)),
                    'includeTransaction.CBChannel.put', (pid, N, t, version, setToInclude, broadcast, receive)).start()
            elif tag == 'A':
                greenletPacker(Greenlet(ACSChannel.put,
                    (sender, m)
                ), 'includeTransaction.ACSChannel.put', (pid, N, t, version, setToInclude, broadcast, receive)).start()
    ...
    for i in range(N):
        greenletPacker(Greenlet(outputCallBack, i),
            'includeTransaction.outputCallBack', (pid, N, t, version, setToInclude, broadcast, receive)).start()
    ...
    greenletPacker(Greenlet(_listener),
        'includeTransaction._listener', (pid, N, t, version, setToInclude, broadcast, receive)).start()
    ...
    greenletPacker(Greenlet(consensusBroadcast, pid, N, t, setToInclude, make_bc_br(pid), CBChannel.get, outputChannel, make_bc_send(pid)),
        'includeTransaction.consensusBroadcast', (pid, N, t, version, setToInclude, broadcast, receive)).start()
    greenletPacker(Greenlet(callBackWrap(acs, callbackFactoryACS()), pid, N, t, version, monitoredIntList, make_acs_br(pid), ACSChannel.get),
        'includeTransaction.callBackWrap(acs, callbackFactoryACS())', (pid, N, t, version, setToInclude, broadcast, receive)).start()
...
@greenletFunction
def honestParty(pid, N, t, version, controlChannel, broadcast, receive, send, B = -1):
    ...
    while True:
	    ...
	    tb1 = time.time() 
            ...
            commonSet, proposals = includeTransaction(pid, N, t, version, proposal, broadcast, includeTransactionChannel.get, send)
