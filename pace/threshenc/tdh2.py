...
print "2/3 = ", (ONE+ONE)/(ONE+ONE+ONE)

print "2*(3^-1) = ", (ONE+ONE) * ((ONE+ONE+ONE) ** (-1))
...
class TDHPublicKey(object):
    ...
    def encrypt(self, m, L):
        ...
        C = (c, L, u, u1, e, f)
        ''''print u
        test = group.serialize(u)
        print test
        print group.deserialize(test)
        #print decodestring(group.serialize(u)[2:])
        print "-----"
        test = serialize(u)
        print test
        print deserialize1(test)'''
        return C
